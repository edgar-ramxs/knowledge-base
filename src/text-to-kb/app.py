...
with st.sidebar:
    st.markdown(
        "_Read the accompanying article [Building a Knowledge Base from Texts: a Full Practical Example](https://medium.com/nlplanet/building-a-knowledge-base-from-texts-a-full-practical-example-8dbbffb912fa)_"
    )
    st.header("What is a Knowledge Base")
    st.markdown(
        "A [**Knowledge Base (KB)**](https://en.wikipedia.org/wiki/Knowledge_base) is information stored in structured data, ready to be used for analysis or inference. Usually a KB is stored as a graph (i.e. a [**Knowledge Graph**](https://www.ibm.com/cloud/learn/knowledge-graph)), where nodes are **entities** and edges are **relations** between entities."
    )
    st.markdown(
        '_For example, from the text "Fabio lives in Italy" we can extract the relation triplet <Fabio, lives in, Italy>, where "Fabio" and "Italy" are entities._'
    )
    ...
    st.markdown(
        "REBEL is a **text2text** model obtained by fine-tuning [**BART**](https://huggingface.co/docs/transformers/model_doc/bart) for translating a raw input sentence containing entities and implicit relations into a set of triplets that explicitly refer to those relations. You can find [REBEL in the Hugging Face Hub](https://huggingface.co/Babelscape/rebel-large)."
    )
    st.header("Further steps")
    st.markdown(
        "Even though they are not visualized, the knowledge graph saves information about the provenience of each relation (e.g. from which articles it has been extracted and other metadata), along with Wikipedia data about each entity."
    )
    st.markdown(
    ...
    st.markdown(
        "If you look closely at the extracted knowledge graphs, some extracted relations are false. Indeed, relation extraction models are still far from perfect and require further steps in the pipeline to build reliable knowledge graphs. Consider this demo as a starting step!"
    )
...
options = ["Text", "Article at URL", "Multiple news articles"]
...
if option == "Text":
    text_options = ["Napoleon", "Kobe Bryant", "Google", "Free text"]
    ...
    text_option = st.selectbox(
        "Choose text option:",
        text_options,
        index=text_options.index(st.session_state.text_option),
    )
    ...
    text = st.text_area(
        "Text:", value=text, height=300, disabled=disabled, max_chars=10000
    )
elif option == "Article at URL":
    url_options = ["Crypto", "Jhonny Depp", "Rome", "Free URL"]
    ...
    url_option = st.selectbox(
        "Choose URL option:",
        url_options,
        index=url_options.index(st.session_state.url_option),
    )
    ...
    news_options = ["Google", "Apple", "Elon Musk", "Kobe Bryant"]
    ...
    news_option = st.selectbox(
        "Use articles about:",
        news_options,
        index=news_options.index(st.session_state.news_option),
    )
...
button_text = "Show KB"
if (option == "Text" and text_option == "Free text") or (
    option == "Article at URL" and url_option == "Free URL"
):
